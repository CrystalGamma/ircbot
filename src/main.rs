...
impl ToJson for JoinUser {
	fn to_json(&self) -> Json {
		let mut obj = serialize::json::Object::new();
...
impl FetchUpJoinDB for JsonJoinDB {
	fn fetch(&self, name: &str) -> Option<JoinUser> {
		let mut file = tryopt!(std::fs::File::open(Path::join(&Path::new(&self.0), &Path::new(name))).ok());
	...
	fn update(&mut self, name: &str, new: JoinStatus) -> String {
		use std::path::Path;
		let mut file = std::fs::File::create(Path::join(&Path::new(&self.0), &Path::new(name))).unwrap();	// FIXME: will overwrite aliases
